<template>
	<div class="supplier">
		<el-card class="el-card">
			...
			<el-table :data="table_data.data">
				...
				<el-table-column label="操作" min-width="180">
					<template v-slot="scope">
						...
						<el-button size="mini" type="text" @click="bindContact(scope.row)">绑定联系人</el-button>
					</template>
				</el-table-column>
			</el-table>
			<div class="block">
			...
			<!--s 绑定联系人窗口     -->
			<el-dialog :visible.sync="bind_contact_visible" title="绑定联系人" width="800px">
				<!-- 添加联系人按钮       -->
				<el-button size="mini" type="primary" @click="addContact()">添加联系人</el-button>
				<el-table
					:data="bind_contact_form.user_list"
					style="width: 100%"
				>
					<el-table-column label="联系人" min-width="180" prop="name">
						<template v-slot="scope">
							<el-input v-model="scope.row.name" placeholder="请填写联系人名字"></el-input>
						</template>
					</el-table-column>
					<el-table-column label="联系电话" min-width="180" prop="phone">
						<template v-slot="scope">
							<el-input v-model="scope.row.phone" placeholder="请填写联系人电话"></el-input>
						</template>
					</el-table-column>
					<el-table-column label="是否已绑定企业微信" min-width="180">
						<template v-slot="scope">
					...
					<el-table-column label="操作" min-width="180">
						<template v-slot="scope">
							<el-button
								size="mini" type="danger" @click="bind_contact_form.user_list.splice(scope.$index, 1)"
							>删除
							</el-button>
						</template>
					</el-table-column>
				...
				<span slot="footer" class="dialog-footer">
					<el-button @click="bind_contact_visible = false">取 消</el-button>
					<el-button type="primary" @click="bindContactSubmit">确 定</el-button>
				</span>
			</el-dialog>
			<!--e 绑定联系人窗口     -->
		</el-card>
...
<script>
import { CreateSupplierUser, GetSupplierList, GetSupplierUserList, OperateSupplier } from '@/api/admin'
...
export default {
	...
	methods: {
		// 打开绑定联系人
		bindContact(item) {
			this.bind_contact_form.user_list = []
			this.bind_contact_form.supplier_id = item.id
			this.getSupplierUserList(item.id)
			this.bind_contact_visible = true
		},
		// 添加联系人
		addContact() {
			this.bind_contact_form.user_list.push({
				name: '',
				phone: '',
				userid: ''
			})
		...
		// 获取供应商联系人列表
		getSupplierUserList(id) {
			GetSupplierUserList({ supplier_id: id })
				.then((res) => {
					this.bind_contact_form.user_list = res.data.items
				})
				.catch((err) => {
		...
		// 确定绑定联系人
		bindContactSubmit() {
			CreateSupplierUser(this.bind_contact_form)
				.then((res) => {
					this.$message.success('绑定成功')
					this.bind_contact_visible = false
				})
				.catch((err) => {
					this.$message.error(err.data.msg)
				})
